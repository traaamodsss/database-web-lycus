...
// Session Config (Penting untuk Vercel)
app.set('trust proxy', 1);
app.use(session({
  secret: 'exofloods-secret-key',
  resave: false,
  saveUninitialized: true,
  cookie: { secure: process.env.NODE_ENV === 'production' }
}));

// --- Helper: Ambil Data GitHub ---
async function fetchData() {
  try {
    const res = await fetch(GITHUB_API_URL, {
      headers: { 'Authorization': `token ${GITHUB_TOKEN}`, 'Accept': 'application/vnd.github.v3+json' }
...
// --- Helper: Update Data GitHub (Wajib Pakai SHA) ---
async function updateData(newData) {
  try {
    const getRes = await fetch(GITHUB_API_URL, {
      headers: { 'Authorization': `token ${GITHUB_TOKEN}` }
    ...
    const putRes = await fetch(GITHUB_API_URL, {
      ...
      headers: { 'Authorization': `token ${GITHUB_TOKEN}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        message: 'Update via Web Dashboard',
        content: base64Content,
        sha: getData.sha
      })
    });
...
app.post('/add', async (req, res) => {
  ...
  if (data.find(i => i.number === number)) {
    ...
    data.push({ number, status: 'active' });
    const success = await updateData(data);
    req.session.message = success ? "Berhasil menambah nomor ✓" : "Gagal simpan ke GitHub! ❌";
  }
  res.redirect('/');
});
...
// Aksi Admin (POST) - Sesuai dengan admin.ejs yang baru
app.post('/delete', async (req, res) => {
  if (!req.session.isAdmin) return res.redirect('/login-admin');
  const { number } = req.body;
  let data = await fetchData();
  data = data.filter(item => item.number !== number);
  const success = await updateData(data);
  req.session.message = success ? "Nomor dihapus ✓" : "Gagal menghapus ❌";
  res.redirect('/admin');
});
...
app.post('/blacklist', async (req, res) => {
  ...
  const { number } = req.body;
  let data = await fetchData();
  data = data.map(i => i.number === number ? { ...i, status: 'blacklist' } : i);
  const success = await updateData(data);
  req.session.message = success ? "Nomor di-blacklist ✓" : "Gagal blacklist ❌";
  res.redirect('/admin');
});
...
app.post('/whitelist', async (req, res) => {
  ...
  const { number } = req.body;
  let data = await fetchData();
  data = data.map(i => i.number === number ? { ...i, status: 'active' } : i);
  const success = await updateData(data);
  req.session.message = success ? "Nomor diaktifkan ✓" : "Gagal whitelist ❌";
  res.redirect('/admin');
});
